...
- **地图独立**: 同页结构 —— 地图升为主视觉 hero 大区, KPI 独立成区。仍在 `/dashboard` 一页内。
...
│   │           雨量热力/气象站/风险范围)                 │
│   ├ 右上浮层: 图例 (红/橙/黄/蓝)                        │
│   └ 底部浮层: 时间轴 scrubber (覆盖在地图上)            │
│        [全部|2020|2021|2022|2023]  [自定义区间]        │
│        ◀ ▶  2022-09-06  ━━━━●━━━  倍速  ⚡最高风险日    │
└─────────────────────────────────────────────────────┘

  KPI STRIP (独立区, 4 卡, 由当天 snapshot 驱动)
  ┌────────┬────────┬────────┬────────┐
  │风险站点 │最大雨量 │窗口风险日│重点区域 │
  └────────┴────────┴────────┴────────┘

  DETAIL PANELS (次级, WeatherLab 皮肤)
  趋势 / 灾种分布 / 等级 / 实时列表 / 传感数据流
```

### 关键改动

1. **地图升为 hero**: `map-section` 移到最上方 (hero banner 之下或替代),
   高度 ~70vh, 占主视觉。图层 toggle 由右上工具栏改为**地图内浮层 chips**,
   图例浮在右上角。
2. **时间轴覆盖在地图底部**: 新建/改造 `AuTimelinePlayer` 为可浮层化的 scrubber:
   - 顶行: 年份快选 pills (全部/2020/2021/2022/2023) + 自定义区间 (两个 date input)
   - 主行: 播放/暂停、上一天/下一天、当前日期 (YYYY-MM-DD)、进度条、倍速、最高风险日
   - 半透明毛玻璃背景 (backdrop-filter), 贴地图底边。
3. **KPI 独立成区**: 从地图下方独立一行, 数据来源改为当天 snapshot 聚合:
   - 风险站点 (当天 comp_level>=1 的站数)
   - 最大 24h 雨量 (当天 max rainfall)
   - 窗口风险日 (窗口内 riskDays, 来自 summary)
   - 重点区域 (当天最高 comp_level 站点所在区县)
4. **细节面板重皮肤**: body-grid 保留趋势/灾种/等级/列表/传感, 调整为
   WeatherLab 风格 (更多留白、细边框、克制配色、统一卡片圆角与阴影)。
...
- [ ] 地图为页面主视觉, 时间轴浮在地图底部, 图层 chips/图例浮层化。
- [ ] KPI 卡独立成区, 数值随回放当天 snapshot 变化。
- [ ] 细节面板呈 WeatherLab 风格。
- [ ] 无地震/暴雨峰值硬编码残留。
